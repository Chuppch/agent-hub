# 滑动窗口业务流程代码实现文档

## 一、概述

### 1.1 文档目的

本文档提供在 Auto Agent 执行流程中集成 `ExecutionHistoryManager` 的完整代码实现指南，包括各节点的修改点、集成步骤和使用示例。

### 1.2 核心目标

- **替换 StringBuilder**：将原有的 `StringBuilder executionHistory` 替换为 `ExecutionHistoryManager`
- **自动压缩机制**：在追加步骤时自动触发压缩，控制历史记录长度
- **分层存储**：实现详细层和摘要层的分层管理
- **最小侵入性**：尽可能减少对现有代码的修改

### 1.3 业务流程概览

```
初始化 ExecutionHistoryManager
   ↓
循环执行（step = 1 to maxSteps && !isCompleted）
   ↓
第一阶段：任务分析（调用 getHistory()）
   ↓
第二阶段：精准执行
   ↓
第三阶段：质量监督
   ↓
更新执行历史（调用 appendStep()）
   ↓
继续下一轮执行
   ↓
循环结束处理（调用 getHistory() 生成总结报告）
```

---

## 二、数据结构修改

### 2.1 DynamicContext 修改

**文件位置：** `DefaultAutoAgentExecuteStrategyFactory.java`

**原代码：**
```java
/** 执行历史记录（累积每轮的分析/执行/监督结果） */
private StringBuilder executionHistory;
```

**修改后：**
```java
/** 执行历史记录管理器（实现滑动窗口和分层存储） */
private ExecutionHistoryManager executionHistoryManager;
```

**完整修改示例：**
```java
@Data
@Builder
@AllArgsConstructor
@NoArgsConstructor
public static class DynamicContext {
    /** 当前执行步骤号 */
    private int step = 1;

    /** 最大任务步骤数（防止无限循环） */
    private int maxStep = 1;

    /** 执行历史记录管理器（实现滑动窗口和分层存储） */
    private ExecutionHistoryManager executionHistoryManager;

    /** 当前任务描述（根据质量监督反馈动态更新） */
    private String currentTask;

    /** 任务完成标志（质量检查通过后设置为 true） */
    boolean isCompleted = false;

    /** AI 客户端配置映射表（key=客户端类型，value=配置信息） */
    private Map<String, AiAgentClientFlowConfigVO> aiAgentClientFlowConfigVOMap;

    /** 动态数据存储（用于节点间传递数据，如 emitter、analysisResult 等） */
    private Map<String, Object> dataObjects = new HashMap<>();

    // ... getter/setter 方法
}
```

---

## 三、各节点集成代码

### 3.1 RootNode - 初始化 ExecutionHistoryManager

**文件位置：** `RootNode.java`

**原代码：**
```java
// 上下文信息
dynamicContext.setExecutionHistory(new StringBuilder());
```

**修改后：**
```java
// 上下文信息 - 初始化执行历史管理器
dynamicContext.setExecutionHistoryManager(new ExecutionHistoryManager());
```

**完整代码示例：**
```java
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    log.info("=== 动态多轮执行测试开始 ====");
    log.info("用户输入: {}", requestParameter.getMessage());
    log.info("最大执行步数: {}", requestParameter.getMaxStep());
    log.info("会话ID: {}", requestParameter.getSessionId());

    // 获取智能体-客户端关联表配置
    Map<String, AiAgentClientFlowConfigVO> aiAgentClientFlowConfigVOMap = 
        repository.queryAiAgentClientFlowConfig(requestParameter.getAiAgentId());

    // 客户端对话组
    dynamicContext.setAiAgentClientFlowConfigVOMap(aiAgentClientFlowConfigVOMap);
    
    // 上下文信息 - 初始化执行历史管理器
    dynamicContext.setExecutionHistoryManager(new ExecutionHistoryManager());
    
    // 当前任务信息
    dynamicContext.setCurrentTask(requestParameter.getMessage());
    
    // 最大任务步骤
    dynamicContext.setMaxStep(requestParameter.getMaxStep());

    return router(requestParameter, dynamicContext);
}
```

---

### 3.2 Step1AnalyzerNode - 获取历史记录

**文件位置：** `Step1AnalyzerNode.java`

**原代码：**
```java
String analysisPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
        requestParameter.getMessage(),
        dynamicContext.getStep(),
        dynamicContext.getMaxStep(),
        !dynamicContext.getExecutionHistory().isEmpty() ? dynamicContext.getExecutionHistory().toString() : "[首次执行]",
        dynamicContext.getCurrentTask()
);
```

**修改后：**
```java
String analysisPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
        requestParameter.getMessage(),
        dynamicContext.getStep(),
        dynamicContext.getMaxStep(),
        dynamicContext.getExecutionHistoryManager().getHistory(),
        dynamicContext.getCurrentTask()
);
```

**说明：**
- `getHistory()` 方法已经处理了首次执行的情况，会返回 `"[首次执行]"`
- 不需要额外的空值检查

**完整代码示例：**
```java
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    log.info("\n === 执行第 {} 步 ===", dynamicContext.getStep());

    // 根据客户端类型 - 获取配置信息
    AiAgentClientFlowConfigVO aiAgentClientFlowConfigVO = 
        dynamicContext.getAiAgentClientFlowConfigVOMap()
            .get(AiClientTypeEnumVO.TASK_ANALYZER_CLIENT.getCode());

    // 构建任务分析提示词
    log.info("\n 阶段1: 任务状态分析");
    String analysisPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
            requestParameter.getMessage(),
            dynamicContext.getStep(),
            dynamicContext.getMaxStep(),
            dynamicContext.getExecutionHistoryManager().getHistory(),  // ← 修改点
            dynamicContext.getCurrentTask()
    );

    // 获取 AgentClient 客户端
    ChatClient chatClient = getChatClientByClientId(aiAgentClientFlowConfigVO.getClientId());

    // 调用客户端执行 - 获取分析结果
    String analysisResult = chatClient
            .prompt(analysisPrompt)
            .advisors(a -> a
                    .param(CHAT_MEMORY_CONVERSATION_ID_KEY, requestParameter.getSessionId())
                    .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 1024))
            .call().content();

    assert analysisResult != null;

    // 解析分析结果 - 流式输出给前端
    parseAnalysisResult(dynamicContext, analysisResult, requestParameter.getSessionId());

    // 将分析结果保存到动态上下文中，供下一步使用
    dynamicContext.setValue("analysisResult", analysisResult);

    // 检查是否已经完成
    if (analysisResult.contains("任务状态: COMPLETED") ||
            analysisResult.contains("完成度评估: 100%")) {
        dynamicContext.setCompleted(true);
        log.info("\n 任务已完成，任务描述: {}", dynamicContext.getCurrentTask());
    }

    return router(requestParameter, dynamicContext);
}
```

---

### 3.3 Step2PrecisionExecutorNode - 移除历史记录追加

**文件位置：** `Step2PrecisionExecutorNode.java`

**原代码：**
```java
// 更新执行历史
String stepSummary = String.format("""
        === 第 %d 步执行记录 ===
        【分析阶段】%s
        【执行阶段】%s
        """, dynamicContext.getStep(), analysisResult, executionResult);

dynamicContext.getExecutionHistory().append(stepSummary);
```

**修改后：**
```java
// 注意：此阶段不追加历史记录
// 历史记录将在 Step3QualitySupervisorNode 中统一追加（包含完整的三个阶段结果）
```

**说明：**
- `Step2PrecisionExecutorNode` 只完成了分析阶段和执行阶段
- 监督阶段的结果还未生成，因此不在此处追加历史记录
- 完整的步骤记录（包含三个阶段）将在 `Step3QualitySupervisorNode` 中统一追加

**完整代码示例：**
```java
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    log.info("\n 阶段2: 精准任务执行");

    // 从动态上下文中获取分析结果
    String analysisResult = dynamicContext.getValue("analysisResult");
    if (analysisResult == null || analysisResult.isEmpty()) {
        log.warn(" 分析结果为空，使用默认执行策略");
        analysisResult = "执行当前任务步骤";
    }

    // 获取客户端配置
    AiAgentClientFlowConfigVO aiAgentClientFlowConfigVO = 
        dynamicContext.getAiAgentClientFlowConfigVOMap()
            .get(AiClientTypeEnumVO.PRECISION_EXECUTOR_CLIENT.getCode());

    // 构建执行提示词
    String executionPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
            requestParameter.getMessage(),
            analysisResult
    );

    // 获取 AgentClient 客户端
    ChatClient chatClient = getChatClientByClientId(aiAgentClientFlowConfigVO.getClientId());

    // 调用客户端执行 - 获取执行结果
    String executionResult = chatClient
            .prompt(executionPrompt)
            .advisors(a -> a
                    .param(CHAT_MEMORY_CONVERSATION_ID_KEY, requestParameter.getSessionId())
                    .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 1024))
            .call().content();

    assert executionResult != null;

    parseExecutionResult(dynamicContext, executionResult, requestParameter.getSessionId());

    dynamicContext.setValue("executionResult", executionResult);

    // 注意：历史记录将在 Step3QualitySupervisorNode 中统一追加
    // 因为此时监督阶段的结果还未生成

    return router(requestParameter, dynamicContext);
}
```

---

### 3.4 Step3QualitySupervisorNode - 追加完整步骤记录

**文件位置：** `Step3QualitySupervisorNode.java`

**原代码：**
```java
// 更新执行历史
String stepSummary = String.format("""
        === 第 %d 步完整记录 ===
        【分析阶段】%s
        【执行阶段】%s
        【监督阶段】%s
        """, dynamicContext.getStep(),
        dynamicContext.getValue("analysisResult"),
        executionResult,
        supervisionResult);

dynamicContext.getExecutionHistory().append(stepSummary);
```

**修改后：**
```java
// 更新执行历史 - 追加完整的三阶段结果
dynamicContext.getExecutionHistoryManager().appendStep(
        dynamicContext.getStep(),
        dynamicContext.getValue("analysisResult"),
        executionResult,
        supervisionResult
);
```

**说明：**
- 这是**唯一**追加历史记录的地方
- 追加的是完整的步骤记录（包含分析、执行、监督三个阶段）
- `appendStep()` 方法会自动触发压缩检查

**完整代码示例：**
```java
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    log.info("\n 阶段3: 质量监督检查");

    // 从动态上下文中获取执行结果
    String executionResult = dynamicContext.getValue("executionResult");
    if (executionResult == null || executionResult.trim().isEmpty()) {
        log.warn(" 执行结果为空，跳过质量监督");
        return "质量监督跳过";
    }

    // 获取客户端配置
    AiAgentClientFlowConfigVO aiAgentClientFlowConfigVO = 
        dynamicContext.getAiAgentClientFlowConfigVOMap()
            .get(AiClientTypeEnumVO.QUALITY_SUPERVISOR_CLIENT.getCode());

    // 构建监督提示词
    String supervisionPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
            requestParameter.getMessage(),
            executionResult);

    // 获取对话客户端
    ChatClient chatClient = getChatClientByClientId(aiAgentClientFlowConfigVO.getClientId());

    // 调用客户端执行 - 获取质量检查结果
    String supervisionResult = chatClient
            .prompt(supervisionPrompt)
            .advisors(a -> a
                    .param(CHAT_MEMORY_CONVERSATION_ID_KEY, requestParameter.getSessionId())
                    .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 1024))
            .call().content();

    assert supervisionResult != null;
    parseSupervisionResult(dynamicContext, supervisionResult, requestParameter.getSessionId());

    // 将监督结果保存到动态上下文
    dynamicContext.setValue("supervisionResult", supervisionResult);

    // 根据监督结果决定是否需要重新执行
    if (supervisionResult.contains(("是否通过: FAIL"))) {
        log.info(" 质量检查未通过，需要重新执行");
        dynamicContext.setCurrentTask("根据质量监督的建议重新执行任务");
    } else if (supervisionResult.contains("是否通过: OPTIMIZE")) {
        log.info(" 质量检查建议优化，继续改进");
        dynamicContext.setCurrentTask("根据质量监督的建议优化执行结果");
    } else {
        log.info(" 质量检查通过");
        dynamicContext.setCompleted(true);
    }

    // 更新执行历史 - 追加完整的三阶段结果
    dynamicContext.getExecutionHistoryManager().appendStep(
            dynamicContext.getStep(),
            dynamicContext.getValue("analysisResult"),
            executionResult,
            supervisionResult
    );

    // 增加步骤计数
    dynamicContext.setStep(dynamicContext.getStep() + 1);

    // 如果任务已完成或达到最大步数，进入总结阶段
    if (dynamicContext.isCompleted() || dynamicContext.getStep() > dynamicContext.getMaxStep()) {
        return router(requestParameter, dynamicContext);
    }

    // 否则进行下一轮执行，返回到Step1AnalyzerNode
    return router(requestParameter, dynamicContext);
}
```

---

### 3.5 Step4LogExecutionSummaryNode - 获取完整历史记录

**文件位置：** `Step4LogExecutionSummaryNode.java`

**原代码：**
```java
// 在 getSummaryPrompt 方法中
String summaryPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
        requestParameter.getMessage(),
        dynamicContext.getExecutionHistory().toString());
```

**修改后：**
```java
// 在 getSummaryPrompt 方法中
String summaryPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
        requestParameter.getMessage(),
        dynamicContext.getExecutionHistoryManager().getHistory());
```

**完整代码示例：**
```java
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    log.info("\n === 执行第 {} 步 ===", dynamicContext.getStep());
    log.info("\n 阶段4: 执行总结分析");

    // 记录执行总结
    logExecutionSummary(dynamicContext.getMaxStep(), 
                       dynamicContext.getExecutionHistoryManager(), 
                       dynamicContext.isCompleted());

    // 生成最终总结报告（无论任务是否完成都需要生成）
    generateFinalReport(requestParameter, dynamicContext);

    log.info("\n === 动态多轮执行结束 ====");

    return "ai agent execution summary completed!";
}

/**
 * 生成最终总结报告
 */
private void generateFinalReport(ExecuteCommandEntity requestParameter, 
                                 DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) {
    try {
        boolean isCompleted = dynamicContext.isCompleted();
        log.info("\n--- 生成{}任务的最终答案 ---", isCompleted ? "已完成" : "未完成");

        AiAgentClientFlowConfigVO aiAgentClientFlowConfigVO = 
            dynamicContext.getAiAgentClientFlowConfigVOMap()
                .get(AiClientTypeEnumVO.RESPONSE_ASSISTANT.getCode());

        String summaryPrompt = getSummaryPrompt(aiAgentClientFlowConfigVO, 
                                                requestParameter, 
                                                dynamicContext, 
                                                isCompleted);

        // 获取对话客户端 - 使用任务分析客户端进行总结
        ChatClient chatClient = getChatClientByClientId(aiAgentClientFlowConfigVO.getClientId());

        String summaryResult = chatClient
                .prompt(summaryPrompt)
                .advisors(a -> a
                        .param(CHAT_MEMORY_CONVERSATION_ID_KEY, requestParameter.getSessionId() + "-summary")
                        .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 50))
                .call().content();

        assert summaryResult != null;
        logFinalReport(dynamicContext, summaryResult, requestParameter.getSessionId());

        // 将总结结果保存到动态上下文中
        dynamicContext.setValue("finalSummary", summaryResult);

    } catch (Exception e) {
        log.error("生成最终总结报告时出现异常: {}", e.getMessage(), e);
    }
}

private static String getSummaryPrompt(AiAgentClientFlowConfigVO aiAgentClientFlowConfigVO, 
                                       ExecuteCommandEntity requestParameter, 
                                       DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext, 
                                       boolean isCompleted) {
    String summaryPrompt;
    if (isCompleted) {
        summaryPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
                requestParameter.getMessage(),
                dynamicContext.getExecutionHistoryManager().getHistory());  // ← 修改点
    } else {
        summaryPrompt = String.format("""
                虽然任务未完全执行完成，但请基于已有的执行过程，尽力回答用户的原始问题：
                
                **用户原始问题:** %s
                
                **已执行的过程和获得的信息:**
                %s
                
                **要求:**
                1. 基于已有信息，尽力回答用户的原始问题
                2. 如果信息不足，说明哪些部分无法完成并给出原因
                3. 提供已能确定的部分答案
                4. 给出完成剩余部分的具体建议
                5. 以MD语法的表格形式，优化展示结果数据
                
                请基于现有信息给出用户问题的答案：
                """,
                requestParameter.getMessage(),
                dynamicContext.getExecutionHistoryManager().getHistory());  // ← 修改点
    }
    return summaryPrompt;
}

/**
 * 输出执行总结信息
 */
private void logExecutionSummary(int maxSteps, 
                                 ExecutionHistoryManager executionHistoryManager, 
                                 boolean isCompleted) {
    log.info("\n📊 === 动态多轮执行总结 ====");
    
    // 注意：如果需要获取总步数，可以通过其他方式获取
    // 因为 ExecutionHistoryManager 不暴露 totalSteps 字段
    // 这里可以根据实际需求调整
    
    if (isCompleted) {
        log.info("✅ 任务完成状态: 已完成");
    } else {
        log.info("⏸️ 任务完成状态: 未完成（达到最大步数限制）");
    }
}
```

---

## 四、修改清单

### 4.1 需要修改的文件

| 文件 | 修改点 | 修改类型 |
|------|--------|----------|
| `DefaultAutoAgentExecuteStrategyFactory.java` | DynamicContext 字段类型 | 字段类型修改 |
| `RootNode.java` | 初始化 ExecutionHistoryManager | 方法调用修改 |
| `Step1AnalyzerNode.java` | 获取历史记录 | 方法调用修改 |
| `Step2PrecisionExecutorNode.java` | 移除历史记录追加 | 代码删除 |
| `Step3QualitySupervisorNode.java` | 追加完整步骤记录 | 方法调用修改 |
| `Step4LogExecutionSummaryNode.java` | 获取完整历史记录 | 方法调用修改 |

### 4.2 详细修改点

#### 修改点 1：DynamicContext 字段类型

**文件：** `DefaultAutoAgentExecuteStrategyFactory.java`

**位置：** 第 47 行

**修改：**
```java
// 原代码
private StringBuilder executionHistory;

// 修改后
private ExecutionHistoryManager executionHistoryManager;
```

---

#### 修改点 2：RootNode 初始化

**文件：** `RootNode.java`

**位置：** 第 36 行

**修改：**
```java
// 原代码
dynamicContext.setExecutionHistory(new StringBuilder());

// 修改后
dynamicContext.setExecutionHistoryManager(new ExecutionHistoryManager());
```

---

#### 修改点 3：Step1AnalyzerNode 获取历史

**文件：** `Step1AnalyzerNode.java`

**位置：** 第 35 行

**修改：**
```java
// 原代码
!dynamicContext.getExecutionHistory().isEmpty() ? dynamicContext.getExecutionHistory().toString() : "[首次执行]"

// 修改后
dynamicContext.getExecutionHistoryManager().getHistory()
```

---

#### 修改点 4：Step2PrecisionExecutorNode 移除追加

**文件：** `Step2PrecisionExecutorNode.java`

**位置：** 第 54-61 行

**修改：**
```java
// 原代码（删除）
// 更新执行历史
String stepSummary = String.format("""
        === 第 %d 步执行记录 ===
        【分析阶段】%s
        【执行阶段】%s
        """, dynamicContext.getStep(), analysisResult, executionResult);

dynamicContext.getExecutionHistory().append(stepSummary);

// 修改后（添加注释说明）
// 注意：历史记录将在 Step3QualitySupervisorNode 中统一追加
// 因为此时监督阶段的结果还未生成
```

---

#### 修改点 5：Step3QualitySupervisorNode 追加步骤

**文件：** `Step3QualitySupervisorNode.java`

**位置：** 第 65-76 行

**修改：**
```java
// 原代码
// 更新执行历史
String stepSummary = String.format("""
        === 第 %d 步完整记录 ===
        【分析阶段】%s
        【执行阶段】%s
        【监督阶段】%s
        """, dynamicContext.getStep(),
        dynamicContext.getValue("analysisResult"),
        executionResult,
        supervisionResult);

dynamicContext.getExecutionHistory().append(stepSummary);

// 修改后
// 更新执行历史 - 追加完整的三阶段结果
dynamicContext.getExecutionHistoryManager().appendStep(
        dynamicContext.getStep(),
        dynamicContext.getValue("analysisResult"),
        executionResult,
        supervisionResult
);
```

---

#### 修改点 6：Step4LogExecutionSummaryNode 获取历史

**文件：** `Step4LogExecutionSummaryNode.java`

**位置：** 第 99 行和第 119 行

**修改：**
```java
// 原代码（两处）
dynamicContext.getExecutionHistory().toString()

// 修改后（两处）
dynamicContext.getExecutionHistoryManager().getHistory()
```

---

## 五、集成步骤

### 5.1 步骤 1：修改 DynamicContext

1. 打开 `DefaultAutoAgentExecuteStrategyFactory.java`
2. 找到 `DynamicContext` 类定义
3. 将 `StringBuilder executionHistory` 改为 `ExecutionHistoryManager executionHistoryManager`
4. 更新对应的 getter/setter 方法（如果使用 Lombok 的 @Data，会自动生成）

### 5.2 步骤 2：修改 RootNode

1. 打开 `RootNode.java`
2. 在 `doApply` 方法中找到初始化 executionHistory 的代码
3. 将 `new StringBuilder()` 改为 `new ExecutionHistoryManager()`
4. 更新方法调用为 `setExecutionHistoryManager()`

### 5.3 步骤 3：修改 Step1AnalyzerNode

1. 打开 `Step1AnalyzerNode.java`
2. 在构建 `analysisPrompt` 的地方
3. 将获取历史记录的代码改为 `getExecutionHistoryManager().getHistory()`
4. 移除原有的空值检查逻辑

### 5.4 步骤 4：修改 Step2PrecisionExecutorNode

1. 打开 `Step2PrecisionExecutorNode.java`
2. 找到追加历史记录的代码（约第 54-61 行）
3. **删除**这段代码
4. 添加注释说明历史记录将在 Step3QualitySupervisorNode 中统一追加

### 5.5 步骤 5：修改 Step3QualitySupervisorNode

1. 打开 `Step3QualitySupervisorNode.java`
2. 找到追加历史记录的代码（约第 65-76 行）
3. 将 `String.format()` 和 `append()` 调用改为 `appendStep()` 方法调用
4. 传入四个参数：step、analysisResult、executionResult、supervisionResult

### 5.6 步骤 6：修改 Step4LogExecutionSummaryNode

1. 打开 `Step4LogExecutionSummaryNode.java`
2. 找到 `getSummaryPrompt` 方法
3. 将两处 `getExecutionHistory().toString()` 改为 `getExecutionHistoryManager().getHistory()`
4. 更新 `logExecutionSummary` 方法的参数（如果需要）

### 5.7 步骤 7：添加导入语句

在所有修改的文件中添加导入语句：

```java

```

---

## 六、注意事项

### 6.1 关键注意事项

1. **唯一追加点**：历史记录只在 `Step3QualitySupervisorNode` 中追加，不要在 `Step2PrecisionExecutorNode` 中追加
2. **首次执行处理**：`getHistory()` 方法已经处理了首次执行的情况，返回 `"[首次执行]"`
3. **自动压缩**：`appendStep()` 方法会自动检查并触发压缩，无需手动调用
4. **线程安全**：`ExecutionHistoryManager` 不是线程安全的，确保在单线程环境中使用

### 6.2 兼容性说明

- **向后兼容**：修改后不影响现有的业务逻辑
- **数据格式**：`getHistory()` 返回的格式与原来的 `StringBuilder.toString()` 类似，但包含摘要层
- **性能影响**：预计可以减少 50-70% 的 token 消耗

### 6.3 测试建议

1. **单元测试**：测试各节点的修改是否正确
2. **集成测试**：测试完整的执行流程
3. **边界测试**：测试首次执行、单步执行、大量步数执行的情况
4. **压缩测试**：验证压缩机制是否正常工作

---

## 七、完整代码示例

### 7.1 修改后的完整流程示例

```java
// ========== RootNode.java ==========
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    // ... 其他代码 ...
    
    // 初始化执行历史管理器
    dynamicContext.setExecutionHistoryManager(new ExecutionHistoryManager());
    
    // ... 其他代码 ...
}

// ========== Step1AnalyzerNode.java ==========
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    // ... 其他代码 ...
    
    // 获取历史记录（自动处理首次执行）
    String history = dynamicContext.getExecutionHistoryManager().getHistory();
    
    // 构建分析器提示词
    String analysisPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
            requestParameter.getMessage(),
            dynamicContext.getStep(),
            dynamicContext.getMaxStep(),
            history,  // 使用获取的历史记录
            dynamicContext.getCurrentTask()
    );
    
    // ... 其他代码 ...
}

// ========== Step2PrecisionExecutorNode.java ==========
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    // ... 执行逻辑 ...
    
    // 注意：不在此处追加历史记录
    // 历史记录将在 Step3QualitySupervisorNode 中统一追加
    
    // ... 其他代码 ...
}

// ========== Step3QualitySupervisorNode.java ==========
@Override
protected String doApply(ExecuteCommandEntity requestParameter, 
                         DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
    // ... 监督逻辑 ...
    
    // 更新执行历史 - 追加完整的三阶段结果
    dynamicContext.getExecutionHistoryManager().appendStep(
            dynamicContext.getStep(),
            dynamicContext.getValue("analysisResult"),
            executionResult,
            supervisionResult
    );
    
    // ... 其他代码 ...
}

// ========== Step4LogExecutionSummaryNode.java ==========
private static String getSummaryPrompt(..., boolean isCompleted) {
    if (isCompleted) {
        summaryPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(),
                requestParameter.getMessage(),
                dynamicContext.getExecutionHistoryManager().getHistory());
    } else {
        summaryPrompt = String.format("""
                ... 提示词内容 ...
                %s
                """,
                requestParameter.getMessage(),
                dynamicContext.getExecutionHistoryManager().getHistory());
    }
    return summaryPrompt;
}
```

---

## 八、验证清单

### 8.1 代码修改验证

- [ ] DynamicContext 字段类型已修改
- [ ] RootNode 初始化代码已修改
- [ ] Step1AnalyzerNode 获取历史代码已修改
- [ ] Step2PrecisionExecutorNode 历史追加代码已删除
- [ ] Step3QualitySupervisorNode 历史追加代码已修改
- [ ] Step4LogExecutionSummaryNode 获取历史代码已修改
- [ ] 所有文件的导入语句已添加

### 8.2 功能验证

- [ ] 首次执行时返回 "[首次执行]"
- [ ] 历史记录正确追加
- [ ] 压缩机制正常工作
- [ ] 摘要层正确生成
- [ ] 最终总结报告正确生成

### 8.3 性能验证

- [ ] Token 消耗明显减少
- [ ] 上下文窗口未溢出
- [ ] 执行效率未降低

---

## 九、常见问题

### 9.1 Q: 为什么不在 Step2PrecisionExecutorNode 中追加历史记录？

**A:** 因为此时监督阶段的结果还未生成，完整的步骤记录应该包含三个阶段（分析、执行、监督）的结果。在 `Step3QualitySupervisorNode` 中统一追加可以确保记录完整性。

### 9.2 Q: getHistory() 方法返回的格式是什么？

**A:** `getHistory()` 返回的格式是：
- 摘要层（如果有）：`步骤X-Y执行摘要: ...`
- 详细层：`=== 第 N 步完整记录 === ...`
- 首次执行：`"[首次执行]"`

### 9.3 Q: 压缩是自动触发的吗？

**A:** 是的，`appendStep()` 方法会自动检查是否需要压缩，如果 `recentDetailedHistory.size() > windowSize`，会自动触发 `compressHistory()` 方法。

### 9.4 Q: 如何调整窗口大小？

**A:** 窗口大小会根据总步数动态调整，也可以通过构造函数自定义：
```java
ExecutionHistoryManager manager = new ExecutionHistoryManager(
    10,  // windowSize
    5,   // compressionBatchSize
    5000 // maxSummaryLength
);
```

---

## 十、总结

### 10.1 核心要点

1. **最小侵入性**：只修改必要的代码，保持原有业务逻辑不变
2. **自动压缩**：压缩机制自动触发，无需手动干预
3. **统一追加**：历史记录只在 `Step3QualitySupervisorNode` 中统一追加
4. **格式兼容**：`getHistory()` 返回的格式与原有格式兼容

### 10.2 预期效果

- **Token 消耗减少**：预计减少 50-70%
- **上下文窗口控制**：避免上下文窗口溢出
- **性能提升**：减少不必要的 token 传输和处理

---

**文档版本：** v1.0  
**最后更新：** 2025-01-03  
**作者：** Chuppch

